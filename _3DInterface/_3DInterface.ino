...
void setup() {
  ...
  // unused pins are fairly insignificant, but pulled low to reduce unknown variables
...
long time(int pin, byte mask) {
  ...
  while(checkTimer() < refresh) {
    // pinMode is about 6 times slower than assigning DDRB directly, but that pause is important
    pinMode(pin, OUTPUT);
    PORTB = 0;
    pinMode(pin, INPUT);
    while((PINB & mask) == 0)
